...
async def get_commits_for_user(project_id: str, user_email: str, start_time: datetime, end_time: datetime) -> list[str]:
    ...
    for repo_url in project["repos"]:
        ...
        try:
            ...
            commits = repo.iter_commits(since=start_time.isoformat(), until=end_time.isoformat())
...
# Updated placeholder for LLM call to include commits
async def get_mood_summary_from_llm(session: aiohttp.ClientSession, avg_emotions: dict, commits: list[str], report_type: str = "Individual") -> str:
    # Format commit messages for the prompt
    commit_log = "\n".join(f"- {msg}" for msg in commits) if commits else "No recent commits found."

    prompt = (
        f"Report Type: {report_type}\n"
        f"Based on the following average emotional state and recent commit messages over a period, "
        f"provide a brief mood summary:\n\n"
        f"Average Emotions:\n{avg_emotions}\n\n"
        f"Recent Commits:\n{commit_log}\n\n"
        f"Summary:"
    )
    print(f"Simulating LLM call ({report_type}) with prompt:\n{prompt}")
    # Simulate LLM response
    await asyncio.sleep(1) # Simulate network delay
    return f"Simulated {report_type} mood summary based on {list(avg_emotions.keys())} and {len(commits)} commits."
        ...
        for project in projects:
            ...
            for user_id in members:
                ...
                if not user_data or not user_data.get("email"):
                    print(f"    Skipping user {user_id}: No email found for commit filtering.")
                    continue
                ...
                last_report = await mood_reports_collection.find_one(
                    {"user_id": user_id, "project_id": project_id}, # Filter for individual report
                    sort=[("report_timestamp", -1)]
                )
                last_report_time = last_report["end_time"] if last_report else datetime.min.replace(tzinfo=timezone.utc)
                ...
                new_emotions_cursor = emotions_collection.find({
                    "user_id": user_id,
                    "timestamp": {"$gt": last_report_time, "$lte": current_processing_time}
                }).sort("timestamp", 1)
                new_emotions_data = await new_emotions_cursor.to_list(length=None)

                if not new_emotions_data:
                    print(f"    No new emotion data found for user {user_id} in project {project_id} since last report.")
                    continue
                ...
                if project_min_start_time is None or start_time < project_min_start_time:
                ...
                average_emotions = {
                    ...
                    for emotion in emotion_sums if emotion_counts[emotion] > 0
                }
                ...
                print(f"    Fetching commits for {user_email} between {start_time} and {end_time}")
                commit_messages = await get_commits_for_user(project_id, user_email, start_time, end_time)
                print(f"    Found {len(commit_messages)} relevant commits.")
                project_all_commit_messages.extend(commit_messages) # Aggregate commits

                # Generate mood summary using LLM, now including commits
                mood_summary = await get_mood_summary_from_llm(session, average_emotions, commit_messages, report_type=f"Individual for {user_data.get('username', 'Unknown')}")
                ...
                new_report_data = {
                    ...
                    "report_type": "individual", # Add report type
                }
                await mood_reports_collection.insert_one(new_report_data)
                print(f"    Stored new individual mood report for user {user_id} in project {project_id}.")
            ...
            if processed_user_count > 0 and project_all_emotion_entries:
                ...
                group_average_emotions = {
                    ...
                    for emotion in group_emotion_sums if group_emotion_counts[emotion] > 0
                }
                print(f"    Calculated group average emotions: {group_average_emotions}")
                print(f"    Total group commits considered: {len(project_all_commit_messages)}")

                # Generate group mood summary using LLM
                group_mood_summary = await get_mood_summary_from_llm(
                    session, group_average_emotions, project_all_commit_messages, report_type="Group"
                )
                ...
                print(f"  Skipping group report for project {project_id}: No new data processed.")
